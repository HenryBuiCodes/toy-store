module.exports = {
  async afterCreate(event) {
                                    ...
                                    <table border="0" cellpadding="0" cellspacing="0" role="presentation" style="border-collapse:separate;width:100%;line-height:100%;">
                                      <tr>
                                        <td align="center" bgcolor="#873CFF" role="presentation" style="border:none;border-radius:10px;cursor:auto;font-style:normal;mso-padding-alt:10px 40px;background:#873CFF;" valign="middle">
                                          <p style="display:inline-block;background:#873CFF;color:#FFFFFF;font-family:Poppins, Arial, sans-serif;font-size:16px;font-style:normal;font-weight:600;line-height:120%;margin:0;text-decoration:none;text-transform:none;padding:10px 40px;mso-padding-alt:0px;border-radius:10px;"> ${discountCode} </p>
                                        </td>
                                      </tr>
                                    </table>
                                  </td>
    ...
    `;
    const allSubscription = await strapi
      .service("api::subscription.subscription")
      .find();

    const resultFilter = allSubscription.results
      .filter((item) => item.CustomerEmail !== null)
      .reduce((acc, curr) => {
        if (!acc.find((item) => item.CustomerEmail === curr.CustomerEmail)) {
          acc.push(curr);
        }
        return acc;
      }, []);
    ...
    resultFilter.forEach((element) => {
      ...
      try {
        strapi.service("api::subscription.subscription").update(element.id, {
          data: {
            CustomerData: {
              discountId: newCustomerDiscountData,
            },
          },
        });
      } catch (error) {
